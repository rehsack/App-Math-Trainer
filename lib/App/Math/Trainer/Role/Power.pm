package App::Math::Trainer::Role::Power;
...
App::Math::Trainer::Role::Power - role for power mathematics
...
require_module 'App::Math::Trainer::Role::VulFrac';    # we only want VulFrac type
           ...
           builder => sub {
               ...
               do
               {
                   $vf = VulFrac->new( num   => int( rand( $_[0] ) + 1 ),
                                       denum => int( rand( $_[0] ) + 1 ) );
               } while ( !App::Math::Trainer::Role::VulFrac::_check_vulgar_fraction($vf) );
               return $vf;
           },
        },
    ];
